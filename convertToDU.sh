...
lua wrap.lua milstart.lua Ares_RemoteV0.9.conf --output yaml --slots databank:type=databank,select=manual warpdrive:type=warpdrive core:type=core engine:type=engine,select=manual radar:type=radar,select=manual --plugins register slots config settings commandhandler ar baseflight flighthud pipes screener menuscreener --name Ares_RemoteV0.9 --minify
...
lua wrap.lua milstart.lua Ares_RemoteV0.9def.conf --output yaml --slots databank:type=databank,select=manual warpdrive:type=warpdrive core:type=core engine:type=engine,select=manual radar:type=radar,select=manual --name Ares_RemoteV0.9dev --dev
